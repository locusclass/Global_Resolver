...
async function buildPresence(): Promise<PresenceProof> {
...
async function buildObjectDraft(payload: CanonicalJsonValue): Promise<LocusObjectDraft> {
  const keyPair = await generateEd25519KeyPair();
  const base = {
    schema_id: "note",
    radius_m: 80,
...
test("credential lifecycle end-to-end", async (t) => {
  ...
  const developerId = parseJson(register.body).developer_id as string;
  ...
  const projectId = parseJson(projectRes.body).project_id as string;
  assert.ok(projectId);
  ...
  const keyId = createdKeyJson.key_id as string;
  const apiSecret = createdKeyJson.api_secret as string;
  assert.ok(keyId);
  assert.ok(apiSecret);
  ...
  const accessToken = parseJson(mint.body).access_token as string;
  ...
  const anchoredObjectId = (parseJson(anchor.body).object as Record<string, unknown>).object_id as string;
  ...
  const rotatedSecret = parseJson(rotate.body).api_secret as string;
  ...
  const newToken = parseJson(newMint.body).access_token as string;
