...
function decodeKey(keyBase64: string, type: "public" | "private") {
...
export async function signEd25519(payloadHash: string, privateKey: string): Promise<string> {
...
export async function createPresencePayloadHash(payload: PresencePayload): Promise<string> {
  return sha256Hex(stableStringify(payload));
}

export async function createObjectDraftHash(payload: ObjectDraftPayload): Promise<string> {
  return sha256Hex(stableStringify(payload));
}

export async function verifyPresenceProof(presence: PresenceProof): Promise<boolean> {
...
export async function verifyObjectDraftSignature(draft: LocusObjectDraft): Promise<boolean> {
...
export function cellIdFromLatLng(lat: number, lng: number, resolution: number): string {
